...
class GSTCalculator(TaxCalculator):
    def __init__(self, cgst: Decimal, sgst: Decimal, igst: Decimal) -> None:
        if isinstance(cgst, float) or isinstance(sgst, float) or isinstance(igst, float):
            raise TypeError("rates must not be float")

        if cgst < Decimal("0") or cgst > Decimal("1"):
            raise ValueError("invalid cgst")

        if sgst < Decimal("0") or sgst > Decimal("1"):
            raise ValueError("invalid sgst")

        if igst < Decimal("0") or igst > Decimal("1"):
            raise ValueError("invalid igst")
